...
| [34.C_array_index](./34.C_array_index)                                                          | C言語側で定義されたグローバルな配列を、Go言語側から安全にアクセス・変更するサンプルです。                                        |
| [35.C_define_constants](./35.C_define_constants)                                                | goでは `#define` で定義されたマクロ定数は見えるけど、マクロは見えないというサンプルです。                                        |
| [36.structs_HostLayout](./36.structs_HostLayout)                                                | `structs.HostLayout` を使用して、安全にポインタキャスト（ゼロコピー）を行うサンプルです。                                        |
| [37.cgo_runtime_metrics](./37.cgo_runtime_metrics)                                              | cgoで利用できる実行時メトリクスについてのサンプルです。                                                                          |
| [38.C_return_errno](./38.C_return_errno)                                                        | C 関数を多値返却形式で呼び出すと、Go コンパイラが呼び出し直後に errno を読み取り、error インターフェースとして返すサンプルです。 |
